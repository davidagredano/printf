...
/*   Updated: 2025/03/23 13:41:30 by dagredan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
static void	insert_conv_rtl(char *dst, const char *src, t_spec *spec)
{
	int	i;
	int	padding;
	int	lead_signs;
 ...
 * if there is room to fill in the precision size before copying the src.
 */
static void	insert_conv_ltr(char *dst, const char *src, t_spec *spec)
{
	int	i;
	int	lead_signs;
...
{
	if (spec->left_align)
		insert_conv_ltr(dst, src, spec);
	else
		insert_conv_rtl(dst, src, spec);
}
