...
/*   Updated: 2025/03/23 13:07:40 by dagredan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
static void	insert_str(char *dst, const char *src, t_spec *spec)
{
	int	i;
 ...
 */
static void	validate_spec(const char *s, t_spec *spec)
{
...
int	print_s(const char *s, t_spec *spec)
{
	...
	if (!s && spec->precision > -1 && spec->precision < 6)
		...
		s = "(null)";
	validate_spec(s, spec);
	str = get_base_str(spec);
	if (!str)
		return (-1);
	insert_str(str, s, spec);
